...
**Status: phase 1, verified end to end.** `pii-service`, the LiteLLM guardrail
plugin, the audit schema and compose wiring. All five done criteria from the
brief have been run against a live stack — see
[`scripts/verify_end_to_end.py`](scripts/verify_end_to_end.py) and the tier-2
result below. No UI — phases 2–5 are sketched in the brief and deliberately not
built.
